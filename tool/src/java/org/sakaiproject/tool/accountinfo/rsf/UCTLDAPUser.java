...
public class UCTLDAPUser {
	...
	private String keystoreLocation = ServerConfigurationService.getString("/usr/local/sakai",
			"accountInfo.ldapKeyStore"); // keystore location (only needed for
											// SSL connections)
	private String keystorePassword = ServerConfigurationService.getString("changeit", "accountInfo.ldapKeyPassword"); // keystore
																														// password
																														// (only
																														// needed
																														// for
	...
	private int operationTimeout = 5000; // default timeout for operations (in
											// ms)
	...
	public UCTLDAPUser(User user) {
		...
		String[] attrList = new String[] { "distinguishedName", "objectClass", "aliasedObjectName", GRACELOGINSTOTAL,
				GRACELOGINSREMAINING, PASSWORDEXPIRATIONTIME };
		Date myDate = null;
	...
	private LDAPEntry getEntryFromDirectory(String searchFilter, String[] attribs, LDAPConnection conn)
		...
		LDAPSearchResults searchResults = conn.search(getBasePath(), LDAPConnection.SCOPE_SUB, searchFilter, attribs,
				false, cons);
